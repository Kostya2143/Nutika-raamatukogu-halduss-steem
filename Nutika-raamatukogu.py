...
class Raamat:
    ...
    def __init__(self, raamatu_id, pealkiri, autor, saadaval="jah"):
        self.raamatu_id = raamatu_id  # raamatu ID
        self.pealkiri = pealkiri      # raamatu pealkiri
        self.autor = autor            # raamatu autor
        self.saadaval = saadaval      # kas raamat on saadaval (jah/ei)


class Kasutaja:
    # Kasutaja klass
    def __init__(self, kasutaja_id, nimi):
        self.kasutaja_id = kasutaja_id  # kasutaja ID
        self.nimi = nimi                # kasutaja nimi


class Laenutus:
    # Laenutuse klass (raamatu laenamine)
    def __init__(self, kasutaja_id, raamatu_id, laenutuse_kuup, tagastus_kuup=""):
        self.kasutaja_id = kasutaja_id        # kes laenas
        self.raamatu_id = raamatu_id          # milline raamat
        self.laenutuse_kuup = laenutuse_kuup  # laenutamise kuupäev
        self.tagastus_kuup = tagastus_kuup    # tagastamise kuupäev
...
def kuva_raamatud():
    ...
    with open(RAAMATUD_FAIL, "r") as f:
        reader = csv.reader(f)
        next(reader)  # jätame päise vahele
    ...
    with open(RAAMATUD_FAIL, "r") as f:
        ...
        for rida in reader:
            if rida[0] == raamatu_id and rida[3] == "jah":
                rida[3] = "ei"  # muudame mitte saadavaks
                leitud = True
